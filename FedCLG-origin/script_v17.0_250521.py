...
def CLG_FedMV(net_glob, global_round, eta, gamma, K, E, M, ratio=0.3, lambda_val=1):
    """
    CLG-SGD+FedMV
    """
...
def FedDU_FedMV(net_glob, global_round, eta, gamma, K, E, M, ratio=0.3, lambda_val=1):
    """
    FedDU+FedMV
    """
    ...
    for round in tqdm(range(global_round)):
        ...
        # 计算FedDU的alpha
        du_C_old = 1
        alpha_du = (1 - acc_t) * (n_0 * D_P_t_prime) / (n_0 * D_P_t_prime + num_current * D_P_0 + epsilon)
        alpha_du = alpha_du * (decay_rate ** round) * du_C_old
        ...
        if alpha_new > 0.001:
            ...
            # final_model = ratio_combine(w_agg, update_server_w, alpha_new)
            # 用du计算的alpha
            final_model = ratio_combine(w_agg, update_server_w, alpha_du)
            net_glob.load_state_dict(final_model)
            # print(f"Round {round}: Server fixing with alpha_new={alpha_new:.4f}")
        else:
            final_model = copy.deepcopy(w_agg)
...
def FedMV(net_glob, global_round, eta, gamma, K, E, M, ratio=0.3, lambda_val=1):
    """
...
def FedAT_FedMut(net_glob, global_round, eta, gamma, K, E, M, ratio=0.3, lambda_val=1):
    """
    ...
    for round in tqdm(range(global_round)):
        ...
        tmp_radius= 4
        # Mutation扩散，为下一轮准备初始模型
        w_locals = mutation_spread(round, w_agg, M, w_delta, tmp_radius)
...
def FedAT(net_glob, global_round, eta, gamma, K, E, M, ratio=0.3, lambda_val=1):
