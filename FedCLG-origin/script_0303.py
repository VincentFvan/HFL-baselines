...
def calculate_server_gradient(model, server_data, batch_size, num_iterations):
    ...
    if origin_model == "resnet" or origin_model == "cnn":
        dataset = TensorDataset(
            torch.Tensor(server_data[0]).to(device),
            torch.Tensor(server_data[1]).long().to(device),
        )
    elif origin_model == "lstm":
        dataset = TensorDataset(
            torch.LongTensor(server_data[0]).to(device),
            torch.Tensor(server_data[1]).to(device),
        )
...
def FedDU_modify(initial_w, global_round, eta, gamma, K, E, M):
    ...
    for round in tqdm(range(global_round)):
        ...
        # # 计算自适应学习率
        # if round > global_round // 2:  # 训练后期减小学习率
        #     current_gamma = max(0.001, gamma * (1 / (1 + 0.01 * (round - global_round // 2))))
        # else:
        #     current_gamma = gamma
        ...
        for cls in unique_classes:
            P_t_prime[cls] = (
                np.sum(selected_client_labels == cls) / n_prime if n_prime > 0 else 0
            )
        ...
        if round % server_update_frequency == 0:
            # 直接计算服务器梯度
            test_model.load_state_dict(w_t_half)
            server_grad = calculate_server_gradient(
                test_model, server_data, batch_size, tau
            )

            # 服务器也执行标准更新，用于评估
            server_update_w, round_loss, _ = update_weights(
                copy.deepcopy(w_t_half), server_data, current_gamma, E
            )
            local_loss.append(round_loss)
            ...
            for key in server_grad.keys():
                if server_grad[key] is not None:  # 确保梯度存在
                    train_w[key] = (
                        w_t_half[key] - tau_eff * current_gamma * server_grad[key]
                    )
