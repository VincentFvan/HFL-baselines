...
def FedDU_modify(initial_w, global_round, eta, gamma, K, E, M):
    ...
    for round in tqdm(range(global_round)):
        ...
        for i, cls in enumerate(unique_classes):
            P_t_prime[i] = (
                np.sum(selected_client_labels == cls) / len(selected_client_labels)
                if len(selected_client_labels) > 0
                else 0
            )
        ...
        alpha = (
            (1 - acc_t)
            * (n_0 * D_P_t_prime)
            / (n_0 * D_P_t_prime + num_current * D_P_0 + epsilon)
        )
        ...
        if alpha > 0.001:
            ...
            # 服务器更新
            update_server_w, round_loss, _ = update_weights(
                copy.deepcopy(w_t_half), server_data, gamma, E
            )
            ...
            print(
                f"Round {round} server fixing with iters/actual_iters/alpha {server_iter}/{actual_iter}/{alpha:.4f}"
            )
        else:
            # 如果alpha不够大，直接使用客户端聚合模型
            train_w = copy.deepcopy(w_t_half)
            # 仍然计算服务器损失用于记录
            _, round_loss, _ = update_weights(
                copy.deepcopy(w_t_half), server_data, gamma, E
            )
            local_losses.append(round_loss)
...
def FedDU_Mut(net_glob, global_round, eta, gamma, K, E, M):
    ...
    for round in tqdm(range(global_round)):
        ...
        if alpha > 0.001:
            ...
            # 服务器更新
            update_server_w, round_loss, _ = update_weights(
                copy.deepcopy(w_agg), server_data, gamma, E
            )
            local_loss.append(round_loss)
            ...
            print(
                f"Round {round}: Server fixing with alpha={alpha:.4f}, iters={server_iter}/{actual_iter}"
            )
        else:
            # 如果alpha不够大，直接使用客户端聚合模型
        ...
        if round % 10 == 0:
            print(
                f"Round {round}: Acc={current_acc:.2f}%, Loss={loss_avg:.4f}, D_P_t={D_P_t_prime:.4f}, Alpha={alpha:.4f}"
            )
...
#     if len(results_test_acc["CLG_Mut"]) >= 20:
...
for du_C_val in du_C_values:
    ...
    # 保存第20轮结果 (第19个索引，因为索引从0开始)
    if len(results_test_acc["FedDU"]) >= 20:
    ...
    plt.tight_layout()
    plt.savefig(
        f"output/ablation/test_accuracy_du_C{du_C_val}_{origin_model}_{timestamp}.png"
    )
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(
        f"output/ablation/train_loss_du_C{du_C_val}_{origin_model}_{timestamp}.png"
    )
    plt.show()
...
timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
du_C_df.to_csv(
    f"output/ablation/du_C_comparison_{origin_model}_{timestamp}.csv", index=False
)
...
plt.tight_layout()
plt.savefig(
    f"output/ablation/du_C_r20_accuracy_comparison_{origin_model}_{timestamp}.png"
)
plt.show()
...
plt.tight_layout()
plt.savefig(
    f"output/ablation/du_C_final_accuracy_comparison_{origin_model}_{timestamp}.png"
)
plt.show()
...
#     plt.grid(True)
#     plt.tight_layout()
#     plt.savefig(f"output/ablation/test_accuracy_decay_rate{decay_val}_{origin_model}_{timestamp}.png")
#     plt.show()
...
#     plt.grid(True)
#     plt.tight_layout()
#     plt.savefig(f"output/ablation/train_loss_decay_rate{decay_val}_{origin_model}_{timestamp}.png")
#     plt.show()
