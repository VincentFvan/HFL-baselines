...
def get_dataset(args):
    ...
    if args.dataset == "mnist":
        ...
        if args.generate_data:
            if args.iid:
                ...
                dict_users = cifar_noniid(
                    dataset_train, args.num_users, args.noniid_case
                )
            else:
                dict_users = separate_data(
                    dataset_train, args.num_users, args.num_classes, args.data_beta
                )
        else:
            dict_users = read_record(file)
        ...
        dataset_train = mydata.CIFAR100_coarse(
            "../data/cifar100_coarse",
            train=True,
            download=True,
            transform=trans_cifar100,
        )
        dataset_test = mydata.CIFAR100_coarse(
            "../data/cifar100_coarse",
            train=False,
            download=True,
            transform=trans_cifar100,
        ...
        # 输出训练集和测试集的大小
        print(f"Origin Training set size: {len(dataset_train)}")
        print(f"Origin Test set size origin: {len(dataset_test)}")
        ...
        dataset_train = datasets.FashionMNIST(
            "./data/fashion-mnist/", train=True, download=True, transform=trans
        )
        dataset_test = datasets.FashionMNIST(
            "./data/fashion-mnist/", train=False, download=True, transform=trans
        )
        if args.generate_data:
            if args.iid:
                dict_users = fashion_mnist_iid(dataset_train, args.num_users)
            else:
                dict_users = fashion_mnist_noniid(
                    dataset_train, args.num_users, case=args.noniid_case
                )
        else:
            dict_users = read_record(file)
    ...
    # 将数据分配的结果记录到file中，保证重复实验数据一样（只要设置args.generate_data=0）
    if args.generate_data:
